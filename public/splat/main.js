...
async function main() {
    ...
    // Load local bahria.splat file
    const url = new URL("../bahria.splat", window.location.origin);
    ...
    window.addEventListener("keydown", (e) => {
        ...
        if (e.code === 'KeyH' && camerasLoaded) {
            carousel = !carousel;
            if (carousel) {
                camid.innerText = "Carousel Mode";
            } else {
            camid.innerText = `Camera ${currentCameraIndex + 1}/${cameras.length}`;
        }
        }
        if (!carousel) {
    ...
    const frame = (now) => {
        ...
        // Handle keyboard controls first
        ...
        // IJKL controls for camera rotation
        if (activeKeys.includes("KeyI")) inv = rotate4(inv, 0.01, 1, 0, 0);
        if (activeKeys.includes("KeyK")) inv = rotate4(inv, -0.01, 1, 0, 0);
        if (activeKeys.includes("KeyJ")) inv = rotate4(inv, -0.01, 0, 1, 0);
        if (activeKeys.includes("KeyL")) inv = rotate4(inv, 0.01, 0, 1, 0);
        ...
        for (let gamepad of gamepads) {
            ...
            if (Math.abs(gamepad.axes[3]) > axisThreshold) {
                ...
                let tiltAxis = (gamepad.buttons[7]?.value || 0) - (gamepad.buttons[6]?.value || 0);
            if (Math.abs(tiltAxis) > axisThreshold) {
                inv = rotate4(inv, rotateSpeed * tiltAxis, 0, 0, 1);
                carousel = false;
            }

                // P and H toggle functionality on B4 and B5
                if (gamepad.buttons[4]?.pressed && !leftGamepadTrigger) {
                    carousel = true;
                    camid.innerText = "";
                }
                if (gamepad.buttons[5]?.pressed && !rightGamepadTrigger && camerasLoaded) {
                    carousel = !carousel;
                    if (carousel) {
                        camid.innerText = "Carousel Mode";
                    } else {
                ...
                leftGamepadTrigger = gamepad.buttons[4]?.pressed || false;
                rightGamepadTrigger = gamepad.buttons[5]?.pressed || false;

                // IJKL mapped to gamepad buttons
                if (gamepad.buttons[0]?.pressed) { // A button (I)
                    inv = rotate4(inv, 0.01, 1, 0, 0);
                carousel = false;
            }
                if (gamepad.buttons[1]?.pressed) { // B button (L)
                    inv = rotate4(inv, 0.01, 0, 1, 0);
                carousel = false;
            }
                if (gamepad.buttons[2]?.pressed) { // X button (K)
                    inv = rotate4(inv, -0.01, 1, 0, 0);
                carousel = false;
            }
                if (gamepad.buttons[3]?.pressed) { // Y button (J)
                    inv = rotate4(inv, -0.01, 0, 1, 0);
                    carousel = false;
                }
            } catch (error) {
        ...
        if (carousel) {
            let inv = invert4(getViewMatrix(cameras[0])); // Use first camera as base
    ...
    // Add message event listener for model loading
    window.addEventListener('message', async (event) => {
        if (event.data.type === 'loadModel') {
            try {
                const response = await fetch(event.data.modelPath);
                const splatData = await response.arrayBuffer();
                selectFile(new File([splatData], 'model.splat', { type: 'application/octet-stream' }));
            } catch (error) {
